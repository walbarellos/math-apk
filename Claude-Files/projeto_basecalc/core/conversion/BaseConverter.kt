...
class BaseConverter(private val base: Int) {
    ...
    fun convert(value: Rational, maxFractionDigits: Int): BaseEntry {
        ...
        val infoExtra = mutableListOf<String>()
        if (base == 10 && value.isInteger()) {
            val vInt = value.toBigIntegerExact()!!
            infoExtra.add(if (vInt.remainder(BigInteger.valueOf(2)) == BigInteger.ZERO) "Par" else "Ímpar")
            
            if (vInt.signum() >= 0) {
                val root = vInt.sqrt()
                if (root.pow(2) == vInt) infoExtra.add("Quadrado perfeito")
            }
        }

        val conjunto = ConjuntoClassifier.classificar(value)
