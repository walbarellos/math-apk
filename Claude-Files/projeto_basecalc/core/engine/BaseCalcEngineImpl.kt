...
class BaseCalcEngineImpl(
    ...
    override fun evaluateExpression(expression: String, sourceBase: Int): CalcResult {
        return try {
            val parser = ExpressionParser(expression, sourceBase)
            ...
            val operationVisual = if (sourceBase != 10) {
                OperationVisualizer.generate(expression, sourceBase)
            } else null
            ...
            CalcResult(
                ...
                bases = bases,
                operationVisual = operationVisual
            )
        } catch (e: Exception) {
            CalcResult(
                ok = false,
